...
public class PrayTime {
    ...
    // ------------------------------------------------------------
    // Calculation Methods
    static final int Jafari = 0; // Ithna Ashari
    static final int Karachi = 1; // University of Islamic Sciences, Karachi
    static final int ISNA = 2; // Islamic Society of North America (ISNA)
    static final int MWL = 3; // Muslim World League (MWL)
    static final int Makkah = 4; // Umm al-Qura, Makkah
    static final int Egypt = 5; // Egyptian General Authority of Survey
    static final int Tehran = 6; // Institute of Geophysics, University of Tehran
    static final int Custom = 7; // Custom Setting
    static final int SIHAT = 8; // Indonesia
    static final int Gulf = 9; // Gulf (UAE, Qatar, Kuwait)
    static final int Russia = 10; // Russia (UFA)
    static final int France = 11; // France (Muslims of France)
    static final int ISNAAlternative = 12; // ISNA Alternative (1-hour Isha in summer)
    static final int Diyanet = 13; // Diyanet (Turkey)
...
//    HANAFI: step=2
    ...
    PrayTime() {
        ...
        timeNames.addAll(Arrays.asList(Constants.KEYS));
        ...
        // Diyanet (Turkey)
        double[] Dvalues = {18, 1, 0, 0, 17};
        methodParams.put(Integer.valueOf(this.getDiyanet()), Dvalues);

        // Gulf (UAE, Qatar, Kuwait)
        double[] Gvalues = {18.2, 1, 0, 0, 19.5};
        methodParams.put(Integer.valueOf(this.getGulf()), Gvalues);

        // Russia (UFA)
        double[] Rvalues = {16, 1, 0, 0, 15};
        methodParams.put(Integer.valueOf(this.getRussia()), Rvalues);

        // France (Muslims of France)
        double[] Fvalues = {12, 1, 0, 0, 12};
        methodParams.put(Integer.valueOf(this.getFrance()), Fvalues);

        // ISNA Alternative (1-hour Isha in summer)
        double[] IAvalues = {15, 1, 0, 1, 60}; // 60 minutes after Maghrib
        methodParams.put(Integer.valueOf(this.getISNAAlternative()), IAvalues);
    }
    ...
    // return prayer times for a given date
    private ArrayList<String> getPrayerTimes(Calendar date, double latitude, double longitude, double tZone) {
    ...
    public static LinkedHashMap<String, String> getPrayerTimes(Context context, double lat, double lng) {
        ...
        prayers.setCalcMethod(settings.getCalcMethodFor(Egypt));
        prayers.setAsrJuristic(settings.getAsrMethodSetFor(SHAFII));
        prayers.setAdjustHighLats(settings.getHighLatitudeAdjustmentFor(NONE));
...
//        int[] offsets = {2, -2, 3, 2, 2, 2, 2}; // {Fajr,Sunrise,Dhuhr,Asr,Sunset,Maghrib,Isha}
    ...
    public static LinkedHashMap<String, String> getPrayerTimesDate(Context context, double lat, double lng, int daysToAdd) {
        AppSettings settings = AppSettings.getInstance(context);
...
//        Log.e("PrayTime", "getPrayerTimesDate:getTime: " + cal.getTime());
//        Log.e("PrayTime", "getPrayerTimesDate:getTimeZone: " + cal.getTimeZone());
//        Log.e("PrayTime", "getPrayerTimesDate:getTimeInMillis: " + cal.getTimeInMillis());
    ...
    // Getter and Setter for Tehran
    public int getTehran() {
        return Tehran;
    }

    // Getter and Setter for Diyanet (Turkey)
    public int getDiyanet() {
        return Diyanet;
    }

    // Getter and Setter for Gulf (UAE, Qatar, Kuwait)
    public int getGulf() {
        return Gulf;
    }

    // Getter and Setter for Russia (UFA)
    public int getRussia() {
        return Russia;
    }

    // Getter and Setter for France (Muslims of France)
    public int getFrance() {
        return France;
    }

    // Getter and Setter for ISNA Alternative (1-hour Isha in summer)
    public int getISNAAlternative() {
        return ISNAAlternative;
    }
